...
// checks to see if the planet items are in order and if ot will provide feedback to the player
   ...
   $('#question').click(function() { //onclick of the solar system tag the loop will run to check if planets are in order
   ...
   for (var i = 0; i < itemOrder.length; i++) { //iteratres through the array to the length of the 'sortablecontainer' list
            ...
            document.getElementById("results").innerHTML = text; //shows text before breakign out of loop
            return false; //breaks our of loop when it sees an item out of order
            }
            else //displays this text when the right ordre is shown
            {
            text = "Here is your next clue: A number with 4 at the end becomes 4 times larger when the number 4 is removed and put in front. What is the number?";
            }
